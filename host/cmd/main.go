...
import (
	...
	"github.com/tucowsinc/tdp-workers-go/host/handlers"
...
func main() {
	...
	if cfg.HealthcheckEnabled {
		...
		go func() {
			log.Info("Starting health check server for host provision worker")
			err = healthCheckServer.Start(context.Background())
			if err != nil {
				log.Fatal("Error occurred while starting health check server for host provision worker", log.Fields{"error": err})
			}
		}()
	}
