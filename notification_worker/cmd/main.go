...
import (
	...
	"github.com/tucowsinc/tdp-workers-go/notification_worker/handlers"
...
func main() {
	...
	bus, err := message_bus.SetupMessageBus(cfg)
	if err != nil {
	...
	service, err := handlers.NewWorkerService(bus, db, tracer)
	if err != nil {
		log.Error("Error creating worker service", log.Fields{
			types.LogFieldKeys.Error: err,
		})
		return
	}
	...
	if cfg.HealthcheckEnabled {
		...
		go func() {
			log.Info("Starting health check server for notification worker")
			err = healthCheckServer.Start(context.Background())
			if err != nil {
				log.Fatal("Error occurred while starting health check server for notification worker", log.Fields{"error": err})
			}
		}()
	}
	...
	err = bus.Consume(queues, consumerOptions) // block until the messagebus quits
	if err != nil {
