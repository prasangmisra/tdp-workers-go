...
import (
	...
	"github.com/tucowsinc/tdp-workers-go/host_updater/handlers"
...
func main() {
	...
	log.Info("Starting host provision updater worker")
	...
	if err != nil {
		log.Error(types.LogMessages.MessageBusSetupFailed, log.Fields{
	...
	if cfg.HealthcheckEnabled {
		...
		go func() {
			log.Info("Starting health check server for host provision updater worker")
			err = healthCheckServer.Start(context.Background())
			if err != nil {
				log.Fatal("Error occurred while starting health check server for host provision updater worker", log.Fields{"error": err})
			}
		}()
	}
