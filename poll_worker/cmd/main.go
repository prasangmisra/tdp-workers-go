...
import (
	...
	"github.com/tucowsinc/tdp-workers-go/poll_worker/handlers"
)
...
func main() {
	...
	service := handlers.NewWorkerService(messagebusServer, db, tracer, cfg)
	...
	if cfg.HealthcheckEnabled {
		...
		go func() {
			log.Info("Starting health check server for poll worker")
			err = healthCheckServer.Start(context.Background())
			if err != nil {
				log.Fatal("Error occurred while starting health check server for poll worker", log.Fields{
					types.LogFieldKeys.Error: err,
				})
			}
		}()
