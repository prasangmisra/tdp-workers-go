...
import (
	...
	"github.com/tucowsinc/tdp-workers-go/domain/handlers"
...
func main() {
	...
	log.Info("Starting domain provision worker")
	...
	if cfg.HealthcheckEnabled {
		...
		go func() {
			log.Info("Starting health check server for domain provision worker")
			err = healthCheckServer.Start(context.Background())
			if err != nil {
				log.Fatal("Error occurred while starting health check server for domain provision worker", log.Fields{"error": err})
			}
		}()
	}
