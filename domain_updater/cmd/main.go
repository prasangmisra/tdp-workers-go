...
import (
	...
	"github.com/tucowsinc/tdp-workers-go/domain_updater/handlers"
...
func main() {
	...
	if cfg.HealthcheckEnabled {
		...
		go func() {
			log.Info("Starting health check server for domain provision updater worker")
			err = healthCheckServer.Start(context.Background())
			if err != nil {
				log.Fatal("Error occurred while starting health check server for domain provision updater worker", log.Fields{"error": err})
			}
		}()
	}
