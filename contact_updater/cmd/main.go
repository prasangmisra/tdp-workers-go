...
import (
	...
	"github.com/tucowsinc/tdp-workers-go/contact_updater/handlers"
...
func main() {
	...
	log.Info("Starting contact provision updater worker")
	...
	log.Info(types.LogMessages.MessageBusSetupSuccess)
	...
	log.Info(types.LogMessages.DatabaseConnectionSuccess)
	...
	if cfg.HealthcheckEnabled {
		...
		go func() {
			log.Info("Starting health check server for contact provision updater worker")
			err = healthCheckServer.Start(context.Background())
			if err != nil {
				log.Fatal("Error occurred while starting health check server for contact provision updater worker", log.Fields{"error": err})
			}
		}()
	}
