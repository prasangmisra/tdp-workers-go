use utf8;
package DesignDB::Schema::Result::Order;

# Created by DBIx::Class::Schema::Loader
# DO NOT MODIFY THE FIRST PART OF THIS FILE

=head1 NAME

DesignDB::Schema::Result::Order

=cut 

use strict;
use warnings;

use Moose;
use MooseX::NonMoose;
use MooseX::MarkAsMethods autoclean => 1;
extends 'DBIx::Class::Core';

=head1 COMPONENTS LOADED

=over 4

=item * L<DBIx::Class::InflateColumn::DateTime>

=back

=cut 

__PACKAGE__->load_components("InflateColumn::DateTime");

=head1 TABLE: C<order>

=cut 

__PACKAGE__->table("order");

=head1 ACCESSORS

=head2 created_date

  data_type: 'timestamp with time zone'
  default_value: current_timestamp
  is_nullable: 1
  original: {default_value => \"now()"}

=head2 updated_date

  data_type: 'timestamp with time zone'
  is_nullable: 1

=head2 created_by

  data_type: 'text'
  default_value: CURRENT_USER
  is_nullable: 1

=head2 updated_by

  data_type: 'text'
  is_nullable: 1

=head2 id

  data_type: 'uuid'
  default_value: gen_random_uuid()
  is_nullable: 0
  size: 16

=head2 tenant_customer_id

  data_type: 'uuid'
  is_foreign_key: 1
  is_nullable: 0
  size: 16

=head2 type_id

  data_type: 'uuid'
  is_foreign_key: 1
  is_nullable: 0
  size: 16

=head2 customer_user_id

  data_type: 'uuid'
  is_foreign_key: 1
  is_nullable: 1
  size: 16

=head2 status_id

  data_type: 'uuid'
  default_value: tc_id_from_name('order_status'::text, 'created'::text)
  is_foreign_key: 1
  is_nullable: 0
  size: 16

=head2 path_id

  data_type: 'uuid'
  default_value: tc_id_from_name('order_status_path'::text, 'default'::text)
  is_foreign_key: 1
  is_nullable: 0
  size: 16

=cut 

__PACKAGE__->add_columns(
  "created_date",
  {
    data_type     => "timestamp with time zone",
    default_value => \"current_timestamp",
    is_nullable   => 1,
    original      => { default_value => \"now()" },
  },
  "updated_date",
  { data_type => "timestamp with time zone", is_nullable => 1 },
  "created_by",
  { data_type => "text", default_value => \"CURRENT_USER", is_nullable => 1 },
  "updated_by",
  { data_type => "text", is_nullable => 1 },
  "id",
  {
    data_type => "uuid",
    default_value => \"gen_random_uuid()",
    is_nullable => 0,
    size => 16,
  },
  "tenant_customer_id",
  { data_type => "uuid", is_foreign_key => 1, is_nullable => 0, size => 16 },
  "type_id",
  { data_type => "uuid", is_foreign_key => 1, is_nullable => 0, size => 16 },
  "customer_user_id",
  { data_type => "uuid", is_foreign_key => 1, is_nullable => 1, size => 16 },
  "status_id",
  {
    data_type => "uuid",
    default_value => \"tc_id_from_name('order_status'::text, 'created'::text)",
    is_foreign_key => 1,
    is_nullable => 0,
    size => 16,
  },
  "path_id",
  {
    data_type => "uuid",
    default_value => \"tc_id_from_name('order_status_path'::text, 'default'::text)",
    is_foreign_key => 1,
    is_nullable => 0,
    size => 16,
  },
);

=head1 PRIMARY KEY

=over 4

=item * L</id>

=back

=cut 

__PACKAGE__->set_primary_key("id");

=head1 RELATIONS

=head2 customer_user

Type: belongs_to

Related object: L<DesignDB::Schema::Result::CustomerUser>

=cut 

__PACKAGE__->belongs_to(
  "customer_user",
  "DesignDB::Schema::Result::CustomerUser",
  { id => "customer_user_id" },
  {
    is_deferrable => 0,
    join_type     => "LEFT",
    on_delete     => "NO ACTION",
    on_update     => "NO ACTION",
  },
);

=head2 order_contacts

Type: has_many

Related object: L<DesignDB::Schema::Result::OrderContact>

=cut 

__PACKAGE__->has_many(
  "order_contacts",
  "DesignDB::Schema::Result::OrderContact",
  { "foreign.order_id" => "self.id" },
  { cascade_copy => 0, cascade_delete => 0 },
);

=head2 order_item_create_domains

Type: has_many

Related object: L<DesignDB::Schema::Result::OrderItemCreateDomain>

=cut 

__PACKAGE__->has_many(
  "order_item_create_domains",
  "DesignDB::Schema::Result::OrderItemCreateDomain",
  { "foreign.order_id" => "self.id" },
  { cascade_copy => 0, cascade_delete => 0 },
);

=head2 order_item_renew_domains

Type: has_many

Related object: L<DesignDB::Schema::Result::OrderItemRenewDomain>

=cut 

__PACKAGE__->has_many(
  "order_item_renew_domains",
  "DesignDB::Schema::Result::OrderItemRenewDomain",
  { "foreign.order_id" => "self.id" },
  { cascade_copy => 0, cascade_delete => 0 },
);

=head2 order_items

Type: has_many

Related object: L<DesignDB::Schema::Result::OrderItem>

=cut 

__PACKAGE__->has_many(
  "order_items",
  "DesignDB::Schema::Result::OrderItem",
  { "foreign.order_id" => "self.id" },
  { cascade_copy => 0, cascade_delete => 0 },
);

=head2 path

Type: belongs_to

Related object: L<DesignDB::Schema::Result::OrderStatusPath>

=cut 

__PACKAGE__->belongs_to(
  "path",
  "DesignDB::Schema::Result::OrderStatusPath",
  { id => "path_id" },
  { is_deferrable => 0, on_delete => "NO ACTION", on_update => "NO ACTION" },
);

=head2 status

Type: belongs_to

Related object: L<DesignDB::Schema::Result::OrderStatus>

=cut 

__PACKAGE__->belongs_to(
  "status",
  "DesignDB::Schema::Result::OrderStatus",
  { id => "status_id" },
  { is_deferrable => 0, on_delete => "NO ACTION", on_update => "NO ACTION" },
);

=head2 tenant_customer

Type: belongs_to

Related object: L<DesignDB::Schema::Result::TenantCustomer>

=cut 

__PACKAGE__->belongs_to(
  "tenant_customer",
  "DesignDB::Schema::Result::TenantCustomer",
  { id => "tenant_customer_id" },
  { is_deferrable => 0, on_delete => "NO ACTION", on_update => "NO ACTION" },
);

=head2 type

Type: belongs_to

Related object: L<DesignDB::Schema::Result::OrderType>

=cut 

__PACKAGE__->belongs_to(
  "type",
  "DesignDB::Schema::Result::OrderType",
  { id => "type_id" },
  { is_deferrable => 0, on_delete => "NO ACTION", on_update => "NO ACTION" },
);


# Created by DBIx::Class::Schema::Loader v0.07049
# DO NOT MODIFY THIS OR ANYTHING ABOVE! md5sum:vUImfT8TOeBpYbmR44eJ1A


# You can replace this text with custom code or comments, and it will be preserved on regeneration
__PACKAGE__->meta->make_immutable;
1;
