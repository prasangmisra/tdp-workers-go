...
DO LANGUAGE plpgsql  $$	
DECLARE dm_source_v varchar :='opensrs' ; 	
DECLARE   tld_v varchar[] := ARRAY['co.in','firm.in','gen.in','in','ind.in','net.in','org.in'] ; 
BEGIN
...
DECLARE   tld_v varchar[] := ARRAY['co.in','firm.in','gen.in','in','ind.in','net.in','org.in']  ; 
BEGIN 
     DELETE FROM  only public.host_addr h
	 USING itdp.host_addr ih
...
BEGIN 
	...
	--UPDATE itdp.DOMAIN SET dm_status=NULL  WHERE     tld = ANY(tld_v) and dm_source=dm_source_v ;
...
DELETE FROM  itdp.host_addr    WHERE dm_source=dm_source_v   AND   tld = ANY(tld_v);
		...
		--opensrs
		/*UPDATE dm_opensrs.domainname SET  itdp_domain_id=NULL   WHERE  tld = ANY(tld_v);
		UPDATE dm_opensrs.nameservers ns SET  itdp_host_id=NULL, itdp_domain_id=NULL  		FROM dm_opensrs.nameservers nss 
		WHERE ns.idx=nss.idx  AND ns.tld = ANY(tld_v);
		
		UPDATE dm_opensrs.contact SET  itdp_contact_id=NULL WHERE    tld = ANY(tld_v);
		UPDATE itdp.tld SET  migration_status=null, result_domain=null,result_contact=null,result_host=null, updated_date=null 
		WHERE    tld_name = ANY(tld_v);*/
		
	/*	--ascio
		UPDATE dm_ascio.domainname SET  itdp_domain_id=NULL   WHERE  tld = ANY(tld_v);
		UPDATE dm_ascio.nameservers ns SET  itdp_host_id=NULL, itdp_domain_id=NULL  
		FROM dm_ascio.nameservers nss 
		WHERE ns.idx=nss.idx  AND ns.tld = ANY(tld_v);
		
		UPDATE dm_ascio.contact SET  itdp_contact_id=NULL WHERE    tld = ANY(tld_v);
		UPDATE itdp.tld SET  migration_status=null, result_domain=null,result_contact=null,result_host=null, updated_date=null 
		WHERE    tld_name = ANY(tld_v);*/
...
--- Clean dm_opensrs ------------------------
/* DELETE  FROM  dm_opensrs.nameservers  ns WHERE  tld = ANY(tld_v);
 DELETE  FROM  dm_opensrs.contact WHERE  tld = ANY(tld_v);
DELETE  FROM   dm_opensrs.domainname WHERE  tld = ANY(tld_v);*/

/*--- Clean dm_ascio ------------------------
DELETE  FROM  dm_ascio.nameservers_   ns USING dm_ascio.domainname_ dn WHERE dn.domainnameid=ns.domainnameid  AND  tld = ANY(tld_v);
DELETE  FROM  dm_ascio.domainname_ WHERE  tld = ANY(tld_v);
--DELETE  FROM  dm_ascio.contact_ WHERE  tld = ANY(tld_v);
DELETE  FROM  dm_ascio.contact_private WHERE  tld = ANY(tld_v);
DELETE  FROM  dm_ascio.domaincontact  dc USING dm_ascio.domainname dn WHERE dn.domainnameid=dc.domainnameid  AND  tld = ANY(tld_v);
DELETE  FROM  dm_ascio.nameservers  ns WHERE  tld = ANY(tld_v);
DELETE  FROM  dm_ascio.contact WHERE  tld = ANY(tld_v);
DELETE  FROM   dm_ascio.domainname WHERE  tld = ANY(tld_v);*/
...
(SELECT count(*) FROM only public.host_addr ha JOIN itdp.host_addr iha ON iha.id=ha.id  AND iha.dm_source=:dm_source_v   AND  iha.tld= ANY(:tld_v) )  AS ha,
...
(SELECT count(*) FROM only itdp.host_addr ha   WHERE ha.dm_source=:dm_source_v   AND  ha.tld= ANY(:tld_v) )  AS ha;
...
--dm_opensrs by tld
SELECT  'dm_opensrs by tld' AS NAME,
(SELECT count(*)   FROM  dm_opensrs.domainname WHERE   tld= ANY(:tld_v))  as d,
(SELECT count(*)   FROM  dm_opensrs.nameservers ns  WHERE   tld= ANY(:tld_v)  )  as h,
(SELECT count(*)   FROM  dm_opensrs.contact WHERE   tld= ANY(:tld_v))  as c;
