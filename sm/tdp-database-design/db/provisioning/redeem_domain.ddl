...
CREATE TABLE provision_domain_redeem (
  ...
  ry_cltrid                   TEXT,
  in_restore_pending_status   BOOLEAN DEFAULT FALSE,
...
CREATE OR REPLACE TRIGGER keep_provision_status_for_retry_tg
  BEFORE UPDATE ON provision_domain_redeem
  FOR EACH ROW WHEN (
...
-- retries the domain redeem order provision
CREATE OR REPLACE TRIGGER provision_domain_redeem_retry_job_tg
