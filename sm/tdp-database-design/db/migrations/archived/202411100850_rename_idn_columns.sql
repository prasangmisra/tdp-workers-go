---- rename idn columns to in order_item_create_domain, provision_domain, domain tables
DO $$
BEGIN
    IF EXISTS(
        SELECT *
        FROM information_schema.columns
        WHERE table_name='order_item_create_domain' and column_name='idn_uname'
    )
      THEN
        ALTER TABLE IF EXISTS order_item_create_domain RENAME COLUMN idn_uname TO uname;
    END IF;

    IF EXISTS(
        SELECT *
        FROM information_schema.columns
        WHERE table_name='order_item_create_domain' and column_name='idn_lang'
    )
      THEN
        ALTER TABLE IF EXISTS order_item_create_domain RENAME COLUMN idn_lang TO language;
    END IF;

	IF EXISTS(
        SELECT *
        FROM information_schema.columns
        WHERE table_name='provision_domain' and column_name='idn_uname'
    )
      THEN
        ALTER TABLE IF EXISTS provision_domain RENAME COLUMN idn_uname TO uname;
    END IF;

    IF EXISTS(
        SELECT *
        FROM information_schema.columns
        WHERE table_name='provision_domain' and column_name='idn_lang'
    )
      THEN
        ALTER TABLE IF EXISTS provision_domain RENAME COLUMN idn_lang TO language;
    END IF;

	IF EXISTS(
		SELECT *
		FROM information_schema.columns
		WHERE table_name='domain' and column_name='idn_uname'
	)
	  THEN
		ALTER TABLE IF EXISTS domain RENAME COLUMN idn_uname TO uname;
	END IF;

	IF EXISTS(
		SELECT *
		FROM information_schema.columns
		WHERE table_name='domain' and column_name='idn_lang'
	)
	  THEN
		ALTER TABLE IF EXISTS domain RENAME COLUMN idn_lang TO language;
	END IF;
END $$;



---- update views that use the idn columns
...
----- update functions that use the idn columns
...
----- update triggers that use the idn columns
