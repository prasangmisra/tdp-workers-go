-- Add the new column to the accreditation table without any constraints
ALTER TABLE accreditation ADD COLUMN registrar_id TEXT;

-- Update the registrar_id in the accreditation table with the clid values from accreditation_epp
UPDATE accreditation
SET registrar_id = ae.clid
FROM accreditation_epp ae
WHERE accreditation.id = ae.accreditation_id;

-- Alter the registrar_id column in the accreditation table to make it NOT NULL
ALTER TABLE accreditation ALTER COLUMN registrar_id SET NOT NULL;
