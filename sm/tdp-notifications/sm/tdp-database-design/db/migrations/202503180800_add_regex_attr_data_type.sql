...
-- Drop type REGEX if it exists.
DROP DOMAIN IF EXISTS REGEX;
...
-- Add a new data type for attributes: REGEX.
INSERT INTO attr_value_type (name,data_type) VALUES ('REGEX','REGEX') ON CONFLICT DO NOTHING;


-- Add a new column to the attr_value table to store the regex value.
ALTER TABLE attr_value ADD COLUMN IF NOT EXISTS value_regex REGEX;
