--
-- table: order_item_update_hosting_certificate
-- description: this table stores certificate information associated with order_item_update_hosting
--
CREATE TABLE order_item_update_hosting_certificate (
    PRIMARY KEY (id)
) INHERITS (hosting_certificate);

--
-- table: order_item_update_hosting
-- description: this table stored attributes of hosting related orders
--
CREATE TABLE order_item_update_hosting (
    hosting_id    UUID NOT NULL REFERENCES hosting(id),
    is_active               BOOLEAN,
    certificate_id          UUID REFERENCES order_item_update_hosting_certificate,
    PRIMARY KEY (id),
    FOREIGN KEY (order_id) REFERENCES "order",
    FOREIGN KEY (status_id) REFERENCES order_item_status,
    CHECK (is_active IS NOT NULL OR certificate_id IS NOT NULL)
...
CREATE TRIGGER a_order_item_update_plan_tg
    AFTER UPDATE ON order_item_update_hosting
    FOR EACH ROW WHEN (
...
CREATE INDEX ON order_item_update_hosting(order_id);
CREATE INDEX ON order_item_update_hosting(status_id);

-- update hosting order planning
CREATE TABLE update_hosting_plan(
    PRIMARY KEY(id),
    FOREIGN KEY (order_item_id) REFERENCES order_item_update_hosting
) INHERITS(order_item_plan, class.audit_trail);

CREATE TRIGGER plan_update_hosting_provision_hosting_tg
    ...
    EXECUTE PROCEDURE plan_update_hosting_provision();
