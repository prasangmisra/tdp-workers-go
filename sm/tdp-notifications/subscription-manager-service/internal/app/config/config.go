...
type Config struct {
	...
	} `yaml:"rmq" validate:"required"`

	DomainsDB      Database `yaml:"domainsdb" validate:"required"`
	SubscriptionDB Database `yaml:"subscriptiondb" validate:"required"`

	Cache struct {
		LifetimeSec int `yaml:"lifetimeSec"`
...
func LoadConfiguration(configPath string) (config Config, err error) {
	...
	viper.SetDefault("rmq.queueName", "subscriptionmanager")
	...
	config.Log.RedactValues = append(config.Log.RedactValues, config.RMQ.Password, config.DomainsDB.Password, config.SubscriptionDB.Password)
