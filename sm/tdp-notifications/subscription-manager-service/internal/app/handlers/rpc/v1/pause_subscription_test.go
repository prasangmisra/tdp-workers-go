...
func TestPauseSubscriptionHandler(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:       "success",
			msg:        &subscription.SubscriptionPauseRequest{},
			requireErr: require.NoError,
			mocksF: func(srvc *handlersmock.IService, mbsrvr *mocks.MockMessageBusServer) {
				resp := &subscription.SubscriptionPauseResponse{}
				...
				mbsrvr.On("Context").
					Return(ctx).Times(1)
				srvc.On("PauseSubscription", ctx, mock.Anything).
		...
		{
			name:       "error - failed precondition",
			msg:        &subscription.SubscriptionPauseRequest{},
			requireErr: require.Error,
			...
			mocksF: func(srvc *handlersmock.IService, mbsrvr *mocks.MockMessageBusServer) {
				...
				mbsrvr.On("Context").
					Return(ctx).Times(1)
				srvc.On("PauseSubscription", ctx, mock.Anything).
					Return(nil, smerrors.ErrStatusCannotBePaused).Times(1)
				mbsrvr.On("ErrorReply", message.ErrorResponse_FAILED_PRECONDITION, mock.Anything, mock.Anything, false, mock.Anything).
					Return(errors.New("failed precondition")).Times(1)
			},
		},
		{
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			err := router.PauseSubscriptionHandler(mbsrvr, tc.msg)
			tc.requireErr(t, err)
		})
	}
}
