...
job "event-enqueue-cron" {
  ...
  group "event-enqueue-cron-instances" {
    task "event-enqueue-cron" {
      driver = "docker"
      template {
        data        = <<EOH
      ...
      config {
        image              = "ghcr.io/tucowsinc/tdp/worker-event-enqueue-cron:${var.image_tag}"
