...
job "domain-purge-cron" {
  ...
  group "domain-purge-cron-instances" {
    task "domain-purge-cron" {
      driver = "docker"
      template {
        data        = <<EOH
      ...
      config {
        image              = "ghcr.io/tucowsinc/tdp/worker-domain-purge-cron:${var.image_tag}"
