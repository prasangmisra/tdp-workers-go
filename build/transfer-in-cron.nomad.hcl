...
job "transfer-in-cron" {
  ...
  group "transfer-in-cron-instances" {
    task "transfer-in-cron" {
      driver = "docker"
      template {
        data        = <<EOH
      ...
      config {
        image              = "ghcr.io/tucowsinc/tdp/worker-transfer-in-cron:${var.image_tag}"
